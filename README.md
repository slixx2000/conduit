...
```powershell
msiexec /i Conduit_1.0.0_x64_en-US.msi /qn      # .msi
.\Conduit_1.0.0_x64-setup.exe /S                # NSIS
```
...
```bash
sudo apt install ./Conduit_1.0.0_amd64.deb     # resolves dependencies; dpkg -i does not
...
```bash
chmod +x Conduit_1.0.0_amd64.AppImage
./Conduit_1.0.0_amd64.AppImage
```
...
- The AppImage *format* needs libfuse2 to self-mount. Ubuntu 22.04+/Mint 21+ ship
  ...
  with `./Conduit_1.0.0_amd64.AppImage --appimage-extract-and-run`.
